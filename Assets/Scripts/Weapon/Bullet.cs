...
public class Bullet : MonoBehaviour
{
    ...
    private void CheckBulletHit()
    {
        ...
        if (Physics.Raycast(_ray, out _hit, _maxDistance, ~_ignoreLayer)) //  делаем проверку попадания с помощью raycast на попадание в какой-либо объект
        {
            HitHandler();
        }
    ...
    private void HitHandler()
    {
        ...
        if (_hit.collider.TryGetComponent(out Rigidbody rigidbody)) // если у объекта, в который мы попали, есть rigidbody
            rigidbody.AddForceAtPosition(transform.forward * _force, _hit.point); // толкаем его в точке попадания пулей по направлению её полета на силу _force
    }
