...
public class Autoshot : MonoBehaviour
{
    ...
    private void Update()
    {
        if (_elapsedTime > _checkTime) // задаем таймер для проверки цели в перекрестье игрока
        {
            CheckTarget();
    ...
    private void CheckTarget()
    {
        if (_currentWeapon.CurrentBullet != null)
        {
            ...
            if (Physics.Raycast(_ray, out RaycastHit hit, _currentWeapon.CurrentBullet.Bullets.MaxDistance, ~_currentWeapon.CurrentBullet.Bullets.IgnoreLayer)
                && hit.transform.root.TryGetComponent(out Enemy enemy) && _currentWeapon.CurrentAmmunition > 0) //  делаем проверку попадания с помощью raycast на попадание в какой-либо объект
            {
