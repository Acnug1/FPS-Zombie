...
public class PlayerShooter : MonoBehaviour
{
    ...
    private void Start()
    {
        ChangeWeapon(_weapons[_currentWeaponNumber]); // активируем текущее оружие игрока
    }
    ...
    private void GetLoadingWeapons()
    {
        ...
        foreach (var weapon in _weaponsStorage.Weapons)
        {
            ...
            weapon.gameObject.SetActive(false); // отключаем все оружие для корректного отображения
        }
    }
    ...
    private IEnumerator Shooting()
    {
        ...
        while (!_onReload && _player.CurrentHealth > 0 && !_player.IsHealing)
        {
            ...
            else if (_currentWeapon.CurrentAmmunition > 0)
            {
                ...
                if (_playerCamera.transform.localEulerAngles.x >= 271 || _playerCamera.transform.localEulerAngles.x <= 90 && _playerCamera.transform.localEulerAngles.x >= 0) // если мы не выходим за границы поворота камеры Math.Clamp(-90; 90)
