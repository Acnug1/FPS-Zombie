...
public class Inflammable : MonoBehaviour
{
    ...
    public void OnStayFire()
    {
        ...
        if (!_burning) // если нет текущего эффекта горения
        {
            _burning = Instantiate(_mainFireEffect, transform.position, Quaternion.Euler(-90, 0, 0), transform); // создаем новый эффект горения внутри объекта
        }
        else if (!_burning.isEmitting)
            _burning.Play(true); // запускаем родительский и дочерные эффекты горения
    }
